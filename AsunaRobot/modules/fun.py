...
import AsunaRobot.modules.fun_strings as fun
from AsunaRobot import DEMONS, DRAGONS, dispatcher
...
def kiss(update, context):
    ...
    if kiss_type == "Gif":
        try:
            temp = random.choice(fun.KISS_GIFS)
            reply_to.reply_animation(temp)
        except BadRequest:
            kiss_type = "Text"
    ...
    if kiss_type == "Sticker":
        try:
            temp = random.choice(fun.KISS_STICKERS)
            reply_to.reply_sticker(temp)
        except BadRequest:
            kiss_type = "Text"
