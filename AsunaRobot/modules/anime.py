...
def kmanga(update: Update, context: CallbackContext):
    message = update.effective_message
    search = message.text.split(' ', 1)
    if len(search) == 1:
        update.effective_message.reply_text('Format : /kmanga< manga name >')
        return
    search = search[1]
    variables = {'search': search}
    json = requests.post(
    ...
    if json:
        ...
        msg = msg[:-2]
        kinfo = json['kurl']
        buttons = [[InlineKeyboardButton("More Info", url=kinfo)]]
        image = json.get("bannerImage", False)
        msg += f"_{json.get('description', None)}_"
        if image:
            try:
...
KMANGA_HANDLER = DisableAbleCommandHandler("kread", kmanga)
...
dispatcher.add_handler(KMANGA_HANDLER)
